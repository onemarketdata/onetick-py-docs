...
Note, that `query_fun` should return a ``Source`` object,
...
Generic aggregations could be also used in ``onetick.py.Source.agg()`` method.
...
* **Parameters:**
  * **query_fun** (*Callable*) – Function that takes ``Source`` as a parameter,
    ...
    For example, ``rename()`` can’t be used.
    If you try to use such methods here, you will get an error during runtime.
  * **bucket_delimiter** (*bool* *,* *default=False*) – When set to `True` an extra tick is created after each bucket.
