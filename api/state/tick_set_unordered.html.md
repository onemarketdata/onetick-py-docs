...
### ``tick_set_unordered(insertion_policy, key_fields, default_value=None, max_distinct_keys=-1, scope='query', schema=None)``
...
* **Parameters:**
  ...
  * **default_value** (``otp.Source``, ``eval query``) – Evaluated query to initialize unordered tick set from.
...
This class should only be created with ``onetick.py.state.tick_set_unordered()`` function
